...
(`.claude/rules/wos-context.md`). Both are regenerated deterministically
from disk state.
...
- Marker format: `<!-- wos:context:begin -->` / `<!-- wos:context:end -->`
...
5. Implement `update_rules_file(root, content)` — write to
   `.claude/rules/wos-context.md`, creating directory if needed
...
- Rules file at `.claude/rules/wos-context.md` exists and is under 50 lines
