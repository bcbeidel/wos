#!/usr/bin/env bash
#
# check_secrets.sh — Scan Bash scripts for committed secrets.
#
# Deterministic Tier-1 Safety check invoked by /build:check-bash-script.
# Scans *.sh files (and extensionless executables with bash shebangs)
# for well-known API key patterns and credential-shaped variable
# assignments, skipping obvious placeholders.
#
# Toolkit convention — not from the ensemble synthesis. Adapted from
# the peer check-rule/scripts/scan_secrets.sh and check-python-script's
# check_secrets.sh. A FAIL finding excludes the file from Tier-2.
#
# Usage:
#   check_secrets.sh <path> [<path> ...]
#
# Paths may be .sh files or directories (top-level files only).
#
# Exit codes:
#   0   no findings
