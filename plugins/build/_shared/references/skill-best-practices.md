...
**Anti-Pattern Guards name failure modes the workflow steps and Key Instructions do not already mandate.** A guard whose body paraphrases a numbered step (`Step N`, `step #N`, `step 3.5`) or repeats a Key Instruction is process echo — it consumes bullet space without earning signal, and creates renumber debt every time the workflow changes. The 2026-05-07 simplification audit identified the pattern across nine SKILLs (rows 9, 11, 13–16). Tier-1 helper `check_guard_step_echo.py` flags the literal step-citation; Tier-2 dimension `check-guards-name-novel-failure` covers the broader paraphrase. Permitted exception: a guard whose substance is a non-obvious primitive-specific failure mode (setuid intent, mutable-ref pinning, declared-but-unused `--dry-run`) that incidentally cites a step number — retain.
